...
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ENVIRONMENT`                                           | `environment: str = "development"`                                                                              | `development`                  | Set explicitly to `production` (prod) / `staging` (staging) in Cloud Run env via `infra/opentofu/cloudrun.tf`. Arms every prod-only Settings validator — see [Operations & runbook](#operations--runbook).                                                                                                                                                               |
...
| `JWT_SECRET`                                            | `jwt_secret: str = "change-me-in-production"`                                                                   | the placeholder above          | **Not currently set in `infra/opentofu/cloudrun.tf` — the Settings field default applies.** This is an operational gap that should be addressed by moving the secret to Secret Manager and mounting it via `secret_key_ref`, following the same pattern as `kanea-db-url` and `agent-api-key-pepper`.                                                                    |
...
| `AGENT_API_KEY_PEPPER`                                  | `agent_api_key_pepper: str = "change-me-in-production-agent-pepper"`                                            | the placeholder above          | Mounted from Secret Manager (`agent-api-key-pepper`) at boot. In any `ENVIRONMENT` other than `development`, the api **refuses to start** if the value is still the committed placeholder — see `app/core/config.py:_check_pepper_set_in_prod`. The dev placeholder lets local dev boot without any pepper provisioning; never use it for anything that mints real keys. |

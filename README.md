...
| **3. IAP IAM allow-list**         | Even after sign-in, IAP checks `roles/iap.httpsResourceAccessor` on the IAP-protected backend. Membership is bound to `var.admin_iap_member` (currently `group:engineering@kanea.ai`); anyone not in that membership gets a 403 here.                                                                                                                                   | `google_iap_web_backend_service_iam_member.admin_panel_accessor` in `infra/opentofu/iap.tf`            |
| **4. Internal-LB-only ingress**   | The Cloud Run service is marked `ingress = INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER`. Direct hits to the auto-generated `*.run.app` URL are refused at Cloud Run's frontend before the container is ever invoked.                                                                                                                                                         | the admin-panel service block in `infra/opentofu/cloudrun.tf`                                          |
...
The same priority drives delegation: a member can delegate a task only to members with a strictly greater numerical priority (i.e., strictly lower rank) — equal-rank delegation is refused. A member at priority 3 cannot delegate to another priority-3 member; only to priority 4 or higher. The reach check is `_enforce_hierarchy` in `apps/api/app/application/tasks/service.py`.
...
- Both `DepartmentService.create` and `DepartmentService.update` orchestrate the transitions transactionally (clear team → set head) so the intermediate state never persists — the head-isolation rule holds at the moment the head is appointed, not just on later edits.
...
A member who owns a source task — its creator, its assignee, or any workspace admin — files a cross-team request anchored to that task via `POST /tasks/{id}/requests`. The standard path (`create_request` in `apps/api/app/application/tasks/service.py`) mints the target task on the requested team immediately and creates a directed task-relation linking source and target (default `BLOCKS`, configurable per request), atomically with the request itself. The request row is recorded as `FULFILLED` and surfaces in the target team's incoming-direction inbox (`GET /teams/{team_id}/requests?direction=incoming` — the default), with the source team's outgoing-direction view available via `?direction=outgoing`. The target team's leadership (`MANAGER`, `LEAD`, the department head if any — falling back to workspace owners for leaderless teams) receives a `CROSS_TEAM_REQUEST` notification deep-linking to the new task; the new task on their board carries a `cross_team_origin` marker so it's visibly distinct from a self-created task. Readers of the source task see the new dependency in the blocked-by panel the moment the request lands. Whether the auto-fulfilment lifecycle should give the target team an explicit accept/reject step is tracked separately as a product decision in [#50](https://github.com/kanea-ai/kanea-monorepo/issues/50).

A separate manual-fulfilment path exists for `PENDING` requests: `POST /requests/{id}/fulfill` and `POST /requests/{id}/reject`, both gated on the **source** task's team leadership (`MANAGER`, `LEAD`, or the department head) or a workspace admin — verified at `_require_team_leadership_for_source` in `apps/api/app/application/tasks/service.py`. The standard `create_request` endpoint does not produce `PENDING` rows itself, so this lifecycle path is reserved for internal/staged flows that explicitly mint `PENDING` requests outside the standard endpoint.
...
| `DATABASE_ECHO`                                         | `database_echo: bool = False`                                                                                   | `false` (also set explicitly in `.env.development`)                                                                         | Not set in `cloudrun.tf` — the Settings default applies. Toggles SQLAlchemy statement echo for local debugging; should never be `true` in prod or staging.                                                                                                                                                                                                                                                                                                                                                                              |
| `JWT_SECRET`                                            | `jwt_secret: str = "change-me-in-production"`                                                                   | `dev-only-secret-do-not-use-anywhere-real` (set in `.env.development`, overriding the class placeholder)[^secret-overrides] | Mounted from Secret Manager (`jwt-secret`) via `secret_key_ref` on the api Cloud Run service. In any `ENVIRONMENT` other than `development`, the api **refuses to start** if the value is still the committed placeholder — enforced by the unified `_check_required_secrets_in_prod` validator in `app/core/config.py`. The same secret signs and verifies every JWT (human / agent / selection / OAuth onboarding), so rotation invalidates every active session atomically; see [Operations & runbook](#operations--runbook).        |
...
[^secret-overrides]: Asymmetry worth knowing about: `JWT_SECRET` gets an explicit override in `.env.development` (so the committed class placeholder doesn't actually load locally), but `AGENT_API_KEY_PEPPER` does not — its class placeholder is what loads for local dev. The prod tripwire treats them identically; only the local-dev loading path differs. If a local rotation produces "works on my machine" confusion, this asymmetry is usually why.
...
# Frontend — apps with a configured Vitest runner expose a `test` nx target.
# Run via nx; adding a new app's runner is a per-app concern.
nx run admin-panel:test
nx run admin-panel:test --watch
```
...
- **source-team leadership** — workspace admin OR a `MANAGER` / `LEAD` on the **source task's** team OR the head of that team's department. Used by the manual cross-team request fulfil/reject endpoints; verified at `_require_team_leadership_for_source` in `apps/api/app/application/tasks/service.py`.
- **admin or dept-head** — workspace admin OR the head of the target member's department (when assigning the member to a team within that department). Used by the team-assignment endpoint to give a department head limited reach within their own department.
...
| PATCH  | `/tenants/members/{id}/team`       | admin or dept-head | Assign team + team_role.                                 |
...
| GET    | `/tasks/{id}/activity`                | JWT                    | Append-only activity log (activity entries only — comments are served from `/tasks/{id}/comments`).                                                                                                                                                                                                                                                                                                                                                       |
...
| GET    | `/teams/{team_id}/requests`           | JWT                    | Team inbox of cross-team requests. `?direction=incoming` (default) lists requests TARGETING this team — the natural inbox view. `?direction=outgoing` lists requests anchored to a source task on this team — the "what have we asked for?" view. `?status_filter` is optional (default: all statuses); under auto-fulfilment new rows are born `FULFILLED`, so a default `PENDING` filter would silently show nothing. Readable by any workspace member. |
...
| `dev`  | **staging**        | `staging.kanea.ai`. From `infra/opentofu/envs/staging.tfvars` (`domain = "staging.kanea.ai"`), wired through `loadbalancer.tf`'s host rules and managed-cert SANs.             |

...
async def list_audit_logs(
    ...
    - Anyone else: empty list (the route guard rejects USER role
      first, so this branch is only hit by unusual admin priorities).

    Pagination uses ``before=<last_row_id>`` cursor for a stable scroll
    even as new rows arrive — the (created_at DESC, id DESC) order
    matches the ``ix_audit_logs_workspace_created`` index.
    """
    return await service.list_for_principal(admin, limit=limit, before=before)
